...
import { getServices, type ServiceItem } from '@/sanity/lib/services';

export default async function Services({ locale }: { locale: string }) {
  const t = await getTranslations('home.services');
  const services: ServiceItem[] = await getServices(locale);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {services.map((service, index) => (
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-br from-white/90 via-white/95 to-[#016aac]/5 dark:from-darklight/90 dark:via-darklight/95 dark:to-[#016aac]/10 rounded-xl pointer-events-none z-0" />

                {/* Top accent line */}
                <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-transparent via-[#016aac] to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10" />
                ...
                <div className="relative h-48 overflow-hidden bg-gray-100 dark:bg-gray-800 z-0">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-[#016aac]/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" />
                </div>
